      ...
      <div class="grid gap-6 lg:gap-14 items-center md:grid-cols-2 justify-between">
        ...
        >
          ...
          <p class="text-start pt-5 pb-4" v-if="featured">
            {{
              featured.client?.quote || featured?.content?.introduction || "—"
            }}
          </p>
          <div
          ...
          >
            <div>
              <img v-if="featured?.client?.avatar"
                class="w-16 h-16 object-cover rounded-full border-4 p-1 bg-[#00000006] shadow-xl"
                :src="featured?.client?.avatar"
              />
            </div>
            <div>
              <h6 v-if="featured?.client?.quote_author" class="text-start text-lg font-semibold">
