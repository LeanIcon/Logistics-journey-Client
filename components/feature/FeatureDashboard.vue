    ...
    >
      ...
      <ul class="space-y-6 feature-list">
        <li v-for="feature in dashboardData?.right_column?.features || [
          ...
          <span><span class="font-medium feature-title">{{ feature.title }}</span> – {{ feature.description }}</span>
        </li>
      </ul>
    </motion.div>
  </section>
...
<script lang="ts" setup>
  ...
  const dashboardData = computed(() => {
    if (pageData.value?.blocks) {
      return pageData.value.blocks.find(block => block.type === 'Dashboard')?.data
    }
    return null
  })
