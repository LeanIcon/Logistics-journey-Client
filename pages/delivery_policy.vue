<template>
  <div class="highest-width policy-wrap">
    <div class="policy-container">
      <main class="policy-main prose prose-blue" v-if="deliveryContent">
      ...
      <!-- ✅ Dynamic TOC (only when backend policy exists) -->
        <aside
          ...
          v-if="deliveryContent && tocItems.length > 0"
        >
          <nav>
            <ul>
              <li v-for="item in tocItems" :key="item.id">
...
const deliveryContent = ref<PolicyData | null>(null)
const tocItems = ref<TocItem[]>([])

const renderedContent = computed(() => {
  return deliveryContent.value ? deliveryContent.value.content : '';
});
...
onMounted(async () => {
  try {
    const response = await getPolicyBySlug('delivery-policy')
    if (response && response.data) {
      deliveryContent.value = response.data
  ...
  if (deliveryContent.value || tocItems.value.length > 0) {
    setupToc()
  }
})
